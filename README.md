...
- Next.js API routes (`backend/app/api/*`) — no separate service, no database.
- Runs on **port 4000** (`npm run dev` / `npm run start`), so port 3000 stays free for the frontend, which runs as its own process/app once built.
...
`GET /docs` (`http://localhost:4000/docs`) serves the real, static Swagger UI distribution — not a Next.js page. `scripts/copy-swagger-ui.js` copies the `swagger-ui-dist` package's assets into `backend/public/docs/` (gitignored, regenerated) and points its initializer at `/openapi.json`; it runs automatically via `predev`/`prebuild`/`prestart`. `next.config.ts` rewrites bare `/docs` to `/docs/index.html`, since Next's static file serving doesn't resolve directory indexes on its own.

`public/openapi.json` is a hand-maintained OpenAPI 3.0 spec — update it by hand when a route's shape changes; it isn't generated from the route code.
...
This backend is its own Next.js app under `backend/`; the frontend is expected to be a separate app/process on port 3000, not part of this `app/` tree. Project-level config (`package.json`, `tsconfig.json`, `next.config.ts`, `.env.local`) stays at the repo root:
...
backend/
  ...
  public/
    openapi.json                # hand-maintained OpenAPI spec backing /docs
    docs/                       # generated by scripts/copy-swagger-ui.js — gitignored
  lib/
    server/
...
scripts/
  copy-swagger-ui.js            # generates backend/public/docs/ from the swagger-ui-dist package
package.json                   # scripts run `next dev|build|start backend -p 4000`
tsconfig.json                  # shared compiler options + `@/*` -> `backend/*`
next.config.ts                 # loads the root .env.local + rewrites /docs -> /docs/index.html
```

`next dev|build|start` take `backend` as the project directory and bind to port 4000 (`package.json` scripts), so Next resolves `app/` and `lib/` from there. `backend/tsconfig.json` exists only to keep the `@/*` alias working with that layout — it extends the root config.
