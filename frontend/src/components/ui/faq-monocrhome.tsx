...
const faqs: Faq[] = [
  {
    question: "What am I actually deciding when I lock a call?",
    answer:
      "One question per window: will BTC or ETH finish this short window at or above where it started (Green / Up) or below it (Red / Down)? You pick a side and an amount, tap once, and you're in. No order book, no leverage, no exchange jargon to learn.",
    meta: "Lock & Reveal",
  },
  {
    question: "Why does the price disappear after I lock?",
    answer:
      "That's the whole point. Once you commit, the live chart is hidden until the window ends — so you stop refreshing every few seconds. You put the phone down, live your life, and come back at 0:00 to see the result.",
    meta: "Zen Mode",
  },
  {
    question: "How much can I lose on a call?",
    answer:
      "Exactly what you stake — never a dollar more. Every lock has a hard cap equal to the amount you chose. A losing outcome token simply redeems for zero; there is no leverage and nothing can liquidate you beyond your stake.",
    meta: "Hard Cap",
  },
  {
    question: "How do I get paid when I win?",
    answer:
      "Claim is part of the flow — winnings don't appear by magic. When the window settles on-chain, your winning result token can be redeemed for collateral with one tap. SOMNIX only ever sends a claim after DreamDEX confirms the window resolved and your side actually won.",
    meta: "Claim",
  },
  {
    question: "Who runs the market and the payouts?",
    answer:
      "DreamDEX Event Contracts on Somnia. We don't create our own betting system — we use the real Up/Down windows, real order-book odds, and real win/lose rules. SOMNIX is the calm decision layer on top: lock, reveal, claim, and same again on the next window.",
    meta: "DreamDEX",
  },
  {
    question: "Can I look around without connecting a wallet?",
    answer:
      "Yes. Watch mode shows the live window, the crowd's Green/Red odds, and recent results with no wallet, no tap, and no claim. Connect a wallet on Somnia testnet only when you're ready to lock a real call.",
    meta: "Watch Mode",
  },
];
...
function FAQ1() {
  ...
  const [hasEntered, setHasEntered] = useState(typeof window === "undefined");
  ...
  return (
    <div className={`font-sans relative w-full overflow-hidden ${palette.surface} bg-transparent text-inherit`}>
      <div className="pointer-events-none absolute inset-0 z-0" style={{ background: palette.aurora }} />
      <div
      ...
      >
        ...
        <header className="flex flex-col gap-8">
          <div className="space-y-4">
            ...
            <h1 className="text-4xl font-semibold leading-tight md:text-5xl text-emerald-400">
              Lock the call. Hide the noise.
            </h1>
            <p className={`max-w-xl text-base ${palette.muted}`}>
              Everything you need to know about locking a directional call on SOMNIX, powered by real DreamDEX markets on Somnia.
            </p>
          </div>
        </header>
