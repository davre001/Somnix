  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* A plain iframe embed, not the TradingView JS widget API: the JS widget's
            own remove() throws "Cannot read properties of null (reading 'parentNode')"
            when its container unmounts (confirmed in production console), which is an
            uncaught error inside a React effect cleanup — it crashes the whole page
            with no boundary to catch it. A full reload per switch is a much smaller
            cost than that. */}
        <iframe
          key={`${symbol}-${interval}`}
          src={iframeSrc}
